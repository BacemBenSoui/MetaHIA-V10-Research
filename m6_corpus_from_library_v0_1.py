"""MetaHIA M6 -- independent fourth-domain corpus (library) v0.1.

Mirrors `m6_corpus_from_organization_v0_1.py`/`m6_corpus_from_supply_chain_v0_1.py`'s
already-validated, non-degenerate mechanism exactly (separate STRUCTURAL
PREDICTION from INDEPENDENT VERIFICATION, via
`corpus/library_verification_claims_v0_1.json`). No change to the mechanism
itself -- only the corpus files and the `record_id` prefix change, to keep
this a genuinely independent fourth domain (library catalogue:
books/authors/publishers/cities/collections) rather than a variant of the
mechanism itself.

The discovery-side topology is deliberately NOT an isomorphic renaming of
any prior domain: 3 authors with uneven book counts (2/1/3), 2 publishers
each serving a different-sized author group, 2 collections each holding
exactly the books of one publisher's authors -- see
corpus/library_facts_v0_1.json's own docstring field.
"""
...
CORPUS_PATH = Path(__file__).resolve().parent / "corpus" / "library_facts_v0_1.json"
CLAIMS_PATH = Path(__file__).resolve().parent / "corpus" / "library_verification_claims_v0_1.json"
        ...
        for start in starts:
            ...
            records.append(
                StructuralOutcomeRecord(
                    record_id=f"libraryv1::{pattern.pattern_id}::{start.ref_id}",
                    rule=pattern,
                    novelty=1.0,
                    redundancy=0.0,
...
__all__ = ["LibraryCorpusReport", "build_library_corpus", "CORPUS_PATH", "CLAIMS_PATH"]
