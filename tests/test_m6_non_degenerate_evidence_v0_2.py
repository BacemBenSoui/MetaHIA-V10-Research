...
and a genuinely non-trivial (neither 0 nor 2) holdout Brier score.
"""
...
def test_v2_all_records_are_grounded_direct_and_length_one():
    report = build_real_corpus_v2()
    assert all(r.provenance == GROUNDED_DIRECT for r in report.records)
    assert all(r.depth == 1 for r in report.records)


def test_v2_exclusions_are_reported_not_silently_dropped():
    report = build_real_corpus_v2()
    assert len(report.excluded_length_over_one) == 55  # every length-2 candidate: no claims exist at that length yet
    assert len(report.excluded_no_verification_claim) == 3  # ENFANT_DE (both directions), PERE_DE forward: no claim authored
...
def test_v2_holdout_brier_is_genuinely_non_degenerate():
    ...
    assert brier == 0.45333333333333337  # pinned for regression detection
