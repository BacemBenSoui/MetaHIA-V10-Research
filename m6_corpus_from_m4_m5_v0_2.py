"""MetaHIA M6 -- non-degenerate evidence mechanism v0.2.
...
CLAIMS_PATH = Path(__file__).resolve().parent / "corpus" / "family_tree_verification_claims_v0_1.json"
        ...
        for start in starts:
            ...
            records.append(
                StructuralOutcomeRecord(
                    record_id=f"realv2::{pattern.pattern_id}::{start.ref_id}",
                    rule=pattern,
                    novelty=1.0,
                    redundancy=0.0,
...
__all__ = ["RealCorpusV2Report", "build_real_corpus_v2", "CORPUS_PATH", "CLAIMS_PATH"]
