...
| JEV-LABELED | 🟡 mesuré cinq fois (22/22 cas contre Jev officiel Sec. 2 ; 22 cas puis 39 cas contre Kev-0.8B réel, Sec. 6ter/6quater ; 13 ordres de critères × 39 cas, Sec. 6sexies ; 5 formulations d'instructions × 39 cas, Sec. 6septies) — **meilleur compromis global ET meilleure calibration mesurée au baseline** (Brier 0,377, ECE 0,137) ; P8.3a montre que ce résultat n'est **pas pleinement invariant à l'ordre des critères** (précision globale 61,5 % à 82,1 % selon l'ordre, σ=7,6 pts ; « conjoint » 4/4 échoue dans 8/13 ordres ; `EPOUX_DE`/`EPOUSE_DE` résiste à la séparation positionnelle, contrairement à l'hypothèse testée) ; **mais P8.3b montre que cette fragilité ne se généralise pas** : la formulation de `instructions` est un axe stable (σ=2,1 pts sur 5 formulations, « conjoint » résiste 5/5, aucune formulation pire que le baseline) -- la faiblesse mesurée de LABELED est donc concentrée sur l'ordre des critères, pas diffuse ; `MERE_DE`/`PERE_DE` JAMAIS confondu sous aucune des 18 perturbations testées à ce jour (13 ordres + 5 formulations) -- candidat par défaut toujours le mieux justifié, mais adoption définitive volontairement pas encore prononcée |
