...
| JEV-STRICT | 🟡 mesuré deux fois (22 cas puis 39 cas après enrichissement du few-shot, `documentation/P8_Jev_Kev_Local_Decision_Model_V0_1.md` Sec. 6ter/6quater) — robustesse adversariale 16,7 % → **63,2 %** en enrichissant le few-shot de 5 à 24 exemples (déficit d'exemples confirmé, pas une limite fondamentale) ; mais précision positive en baisse (62,5 % → 50,0 %), confusion systématique nouvelle `PERE_DE`→`MERE_DE`, et échec persistant (2 fois) sur `EPOUX_DE`/`EPOUSE_DE` et sur « conjoint » (0/4 paraphrases) ; seule condition validant réellement l'hypothèse d'abstraction pour un LLM, mécanisme réel mais encore immature |
| JEV-LABELED | 🟢 mesuré trois fois (22/22 cas contre Jev officiel Sec. 2 ; 22 cas puis 39 cas contre Kev-0.8B réel, Sec. 6ter/6quater) — **meilleur compromis global ET meilleure calibration mesurée** (Brier 0,377, ECE 0,137, les deux meilleurs des trois conditions) ; seule condition à résister à « conjoint » sur 4/4 paraphrases ; confond encore `EPOUX_DE`/`EPOUSE_DE` (sens inverse de STRICT) — **reclassé** depuis « strict » implicite vers LABELED par ce document |
